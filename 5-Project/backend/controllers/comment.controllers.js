...
export const addComment = async (req, res) => {
 try {
   ...
   if(!content?.trim()){
     res.status(400).json({success: false, message: "Comment field is required"})
   }
   ...
   res.status(200).json({success: true, message: "Comment added successfully", data: commentLocalPath})
 } catch (error) {
  res.status(500).json({success: false, message: "Error while in add Comment"})
 }
}
